...
4a. **Autonomous resolution of un-acted executive meetings** (Executive Delegation & Trust arc, Tier 1, 2026-07-12) — `resolveAutonomousExecMeetings`, runs immediately after the player-meeting loop and before artist-change flush. Every exec-lane meeting the player did NOT personally staff this week never lapses: the engine re-derives what that exec was offered (the same seeded selection pipeline the `/api/roles` route uses) and resolves it autonomously — the exec spends real money per their loyalty band (direction) and mood (risk-appetite tie-break), through the same effect-application path a player pick uses. The exec is marked used (`lastActionWeek` set), which suppresses this week's idle loyalty decay. CEO meetings are exempt (no exec row; the CEO lane genuinely lapses if unspent — see the three-lane rule in the `[REFERENCE]` doc §10). No RNG-stream impact (isolated seeds only). **Exec absence (engine-verbs Slice 11/M7, 2026-07-12):** an executive benched by an authored `set_exec_absence` effect (`flags.execAbsence[roleId] = untilWeek`) is skipped at BOTH sites — the `/api/roles` meeting offer returns no meetings for the absent role (`server/routes/executives.ts` ~49–64) and `resolveAutonomousExecMeetings` excludes them from its candidates (`game-engine.ts` ~739–745) — so an absent exec neither offers a meeting nor auto-resolves one until the absence expires; both sites read the same flag, so player and engine always agree the exec sat out.
...
8a. **Escalation emission** (Tier 2, 2026-07-12) — `applyEscalation`, runs after any prior-week pending crisis has been resolved/cleared and before this week's random-event roll (step 9). If step 4a captured an urgent (reactive) meeting that a below-loyalty-ceiling executive self-resolved this advance, escalation sets `flags.pending_side_event` for the FOLLOWING week via the same mandatory-crisis pipeline step 9 uses — roll-free, zero RNG draws. Running before the weekly roll means escalation wins any same-week collision by construction (the roll's own "already pending" check then discards). One crisis at a time, same rule as the roll. **Last-seen filter (engine-verbs Slice 14/M12b, 2026-07-12):** each landed escalation stamps `flags.escalationHistory[roleId]` (mirrors `flags.side_event_history`); `pickEscalationEventId` filters already-seen ids out of the role's pool before its isolated seeded pick — never to empty (a fully-seen pool falls back to the whole pool, and the stamp saturation-resets the history to just the latest event so the cycle restarts without an immediate repeat).
8b. **Scheduled-event promotion** (engine-verbs Slice 1/M4, 2026-07-12) — `promoteScheduledEvents`, runs immediately after 8a and before the weekly roll (step 9). Drains the `flags.scheduled_events[]` queue (banked by authored `schedule_event` choice effects — value `{event_id, defer_weeks}`): the earliest DUE entry (lowest `landsOnWeek` ≤ current week, FIFO tie-break) promotes into `flags.pending_side_event` as a mandatory crisis, at most ONE per advance. If the slot is already occupied (an 8a escalation, or an unresolved prior crisis), everything WAITS — entries are never dropped, so escalations keep absolute priority and a heavy week can starve the queue by design. Running before step 9 means a promoted event beats the weekly roll the same first-set-wins way escalation does. Legacy (non-mandatory) mode never promotes. `scheduled_only: true` events are excluded from step 9's roll pool (parallel to `escalation_only`). Roll-free, zero RNG draws.
9. Roll random events (in mandatory-side-events mode the roll is **deferred** — it lands on `flags.pending_side_event` as a crisis to resolve next week rather than surfacing an interactive beat this week; one crisis at a time; skipped entirely if step 8a already set a pending crisis this advance — or step 8b already promoted a scheduled event)
