...
**Status**: 🔶 **PARTIALLY RESOLVED** (July 26, 2026) — the safe slice shipped: `summary.chartGenerationFailed` is now set on the swallow path (mirroring C55's `emailGenerationFailed`), so a chartless committed week is at least visible to the client/summary surfaces; new DB-free test `tests/engine/chart-generation-failure-flag.test.ts`. Swallow semantics deliberately unchanged. **Remaining scope**: the rethrow-vs-log-and-continue decision — see `docs/01-planning/PENDING-DECISIONS.md` #11 (the same open ruling as C55; rethrow would abort/roll back the week and re-arm the D6 failure-injection surface). Stays in the Pending bucket like C55.
**Priority**: 🟢 Medium
