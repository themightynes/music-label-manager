...
- Use `npm test` to run tests in watch mode, or `npm run test:run` to run all tests once (CI mode). You can also use `npx vitest [optional/path/to/test/file]` to run specific test files.
