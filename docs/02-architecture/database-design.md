...
- **Artist Energy** (`artists.energy`): Surfaced in UI, narrative beats, and notifications, and — as of the balance-integrity + volatility-economy arcs (July 2026) — also live in the economy: it multiplies tour sell-through (×0.90–1.05 linear in energy, `computeEnergyFactor` in `shared/engine/FinancialSystem.ts`, config at `markets.json` `market_formulas.tour_revenue.energy_effectiveness`) applied consistently across tour foreshadow, execution, and the `/api/tour/estimate` route. Energy is also **consumed and restored over the campaign** (it is no longer a static stat): touring drains a flat amount per revealed city (C87, `markets.json` `tour_revenue.energy_cost.per_city`), an artist with a recording project in `production` loses `energy_lifecycle.recording_drain_per_week`, and an artist with no active project recovers `energy_lifecycle.idle_recovery_per_week` — all accumulated into `summary.artistChanges` and clamped 0–100 by `ArtistStateProcessor`. It still does **not** affect song quality or streaming outcomes — those read `artist.mood`, not `artist.energy`.
