...
#### **2.1 Release planning service — Buzz v2 transaction semantics** *(added July 25, 2026, C84 doc-sync; source: `server/services/releasePlanningService.ts`)*

`releasePlanningService` backs two release-lifecycle endpoints whose behavior changed in the Buzz v2 arc (PR #152) and the July 25, 2026 C83 fix:

- **`POST /api/game/:gameId/releases/plan`** — one transaction that, beyond budget validation/deduction, song reservation, and release-row creation, now also: (1) validates an optional `preCampaignPct` (0 or a 10-step share up to the `pre_release_campaign` balance knob's `max_pct`; nonzero requires ≥1 lead-up week) and, when > 0, stores a `metadata.preCampaign` block (`pct`, `totalBudget` = pct% of the main marketing total, `budgetPerChannel`, `spentToDate`) — pct 0 stores nothing, keeping the legacy path byte-identical; (2) **attaches banked hype at plan time**: the artist's per-artist hype pool plus the entire label pool (`flags.pendingAwarenessBoost`, first-planned-takes-all) are moved onto `metadata.attachedHype` and the flags pools zeroed in the same `gameStates` update.
- **`DELETE /api/game/:gameId/releases/:releaseId`** — planned releases only (`CANNOT_DELETE_RELEASED` 400 otherwise). One transaction: frees the release's songs (`releaseId = null`) **and zeroes their `awareness`/`peak_awareness`** (fork E — built pre-buzz dies with cancellation; attached hype dies implicitly with the deleted row), credits the refund to `gameStates.money`, and deletes the release row. **Refund rule**: `refund = release.marketingBudget − converted pre-campaign share (spentToDate clamped into [0, preCampaign.totalBudget]) − the stored lead-single budget share IF the lead single's song row is already released (C83, July 25, 2026)`, floored at 0 — computed strictly from STORED release data, never from client input. The client's cancel-confirmation preview (`summarizeCancelRelease` in `client/src/lib/releaseBuzz.ts`) mirrors the same subtraction rules for parity; the server's returned `refundedAmount` stays the source of truth. (Known defect C107: the song-zeroing update also hits an already-shipped lead single — see the technical-debt backlog.)
