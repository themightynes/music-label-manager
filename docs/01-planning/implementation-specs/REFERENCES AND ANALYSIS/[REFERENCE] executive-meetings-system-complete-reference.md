...
**Maintenance note**: this document must be updated whenever `LIVE_EFFECT_KEYS` (`shared/engine/processors/ActionProcessor.ts`) or `data/actions.json` changes. The tripwire is `tests/engine/data-lint-effect-keys.test.ts` — if it goes red, an authored key drifted from what the engine implements, and both the data and this doc need reconciling.

---

## 1. System overview + player-facing flow

1. **Exec/meeting selection** — `GET /api/roles/:roleId` (`server/routes/executives.ts:12`) strips `TEST_`-prefixed meetings (`:32`) and deterministically picks **one meeting per executive per week** via `seededRandomPick(roleMeetings, seed)` (`:45`), seed = `gameId-week-roleId` (`shared/utils/seededRandom.ts`, `generateMeetingSeed`).
...
|---|---|---|---|---|
| **ceo_priorities** (global) — "We can only push two tracks next week—what's the focus?" | `studio_first` | money −2000 | quality_bonus +5, artist_mood +2 | Buy studio time now, bank quality for the next session |
...
- The original plan (`[READY] executive-meetings-revival-plan.md` §0.1) named the canonical channel key as `awareness_boost` — this matches the shipped code exactly (no drift there), but the plan's proposed canonical set also mentioned keeping `press_momentum` and both are indeed live; no discrepancy, noted here only because the plan's key list undersold that `rep_swing` also became a full 14th mechanism-bearing key rather than "existing six + executive_mood" as its intro paragraph summarized.
