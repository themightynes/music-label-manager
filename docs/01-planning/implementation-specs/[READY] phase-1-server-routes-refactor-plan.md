...
|---|---|---|---|---|
| 1 | `test: route-manifest characterization for registerRoutes` | New test builds an `express()` app, calls `registerRoutes(app)`, walks `app._router.stack` (recursing into mounted routers), asserts the sorted list of `{method, path, middlewareNames[]}` (~86 entries; middleware names via `fn.name`) against an inline manifest. Add `supertest` + `@types/supertest` devDeps and `tests/helpers/test-app.ts` that mocks `server/auth` to inject `req.userId` (reused by later characterization tests). No production code changes | Low | Trivial `package.json` conflict with #36 — rebase. Importing routes.ts transitively imports `server/db` — tests/setup.ts already provides the DB env |
