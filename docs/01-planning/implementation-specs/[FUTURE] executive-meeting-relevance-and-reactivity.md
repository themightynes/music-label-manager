...
> **Status:** FUTURE / seed planning doc — problem framing + directions. **Scope fork DECIDED July 5, 2026 (Nes): Tier 0+1 as one planned slice** — see `COMPLETED/[COMPLETE] meeting-relevance-tier0-1-plan.md` for the implementation plan (EXECUTED July 5, 2026 — Tier 0+1 shipped as PRs #122–#124); Tier 2 stays deferred, paired with side-events (PENDING-DECISIONS.md §1). This doc remains the problem framing / design rationale. Note: the golden-master caution in "Constraints" below turned out not to apply — selection lives in the roles route, outside the advance-week engine path (plan §0).
