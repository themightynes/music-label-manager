...
- **Successor doc:** `implementation-specs/[FUTURE] meeting-content-working-session-agenda.md` (added 2026-07-12) is the scheduled agenda for this session — it folds in the round-2/round-3 playtest finding that meeting/choice stakes are the root cause the above items all sit downstream of, plus C92 (outcome-summary field) and the flop-threshold / Creative-Capital-economy design decisions (`PENDING-DECISIONS.md` #8/#9).
