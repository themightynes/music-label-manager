...
> **Triaged 2026-07-25** (debt-cleanup session, C59): every item below carries a dated disposition. `[TRIAGED …]` items were re-verified live but deliberately stay in this list rather than the technical-debt backlog.

1. **`updateGameState` dead code + latent musicLabel clobber** — `client/src/store/gameStore.ts:640-701`; PATCH response (`server/routes/games.ts:88`) lacks `musicLabel`, so branch 1 would strip the label from the store and from subsequent saves. *(Fixed by PR-2 via deletion.)* **[RESOLVED — PR-2 deletion; re-verified gone 2026-07-25]**
2. **Unused variable** `availableSlots` — `gameStore.ts:706`. **[RESOLVED — re-verified gone 2026-07-25]**
3. **Full-bundle refetch for one array**: `useProjects.ts:46` and `useArtists.ts:49` each GET the entire `/api/game/:id` payload to select `.projects`/`.artists`; a mutation invalidating both keys triggers two full-bundle GETs. Needs dedicated endpoints (server change → future phase). **[TRIAGED 2026-07-25 — still live at the same lines; remains in this list, not ledger-tracked (needs dedicated server endpoints)]**
4. **Dual bootstrap paths**: `GameContext.tsx:38` (`GET /api/games`, most-recent-game recovery) and `GamePage.tsx:42` (`GET /api/game-state` fallback) overlap; `client/CLAUDE.md:43` describes only the latter. Consolidation candidate. **[TRIAGED 2026-07-25 — still live (GameContext.tsx:38 / GamePage.tsx:44); remains in this list, not ledger-tracked]**
5. **Zustand still owns snapshot-input arrays** that have cache-owned read paths: `emails` (`gameStore.ts:246`, read path is `useEmails`), `executives`/`moodEvents` (`:247-248`, `useExecutives` exists since PR-8). Dual copies exist solely for `saveGame`/`handleExport`; they should source from the cache like the other five collections. **[TRIAGED 2026-07-25 — partially improved: `buildGameSnapshot` now takes snapshot-fetched emails (via `fetchSnapshotCollections`) and snapshot-fetched executives/moodEvents with the store copies as fallback only; the store arrays still exist. Remains in this list, not ledger-tracked]**
6. **`advanceWeek` merge precedence** `{...resultGameState, ...serverGameState}` (`gameStore.ts:872-874`) silently discards any advance-week response field that the immediate refetch disagrees with, and hardcodes `usedFocusSlots: arOfficeSlotUsed ? 1 : 0` (:877) — correct today only because actions clear on week end. **[TRIAGED 2026-07-25 — still live (merge + hardcoded `usedFocusSlots` now at `gameStore.ts:1076-1079`); remains in this list, not ledger-tracked]**
7. **Stale comment** referencing store-owned data — `client/src/lib/releaseAnalytics.ts:3`. **[RESOLVED — the header comment now describes the query cache; re-verified 2026-07-25]**
8. **Heavy `console.log` debug noise** in store hot paths — `gameStore.ts:323-330` (loadGame), `:586-599` (createNewGame), `:834-865` (advanceWeek) — should move behind the `logger` used by `queryClient.ts`. **[TRIAGED 2026-07-25 — still live (36 `console.log` in gameStore.ts); remains in this list, not ledger-tracked]**
9. **`GamePage` effect dep list** includes `createNewGame` which the effect never calls — `GamePage.tsx:80`. **[TRIAGED 2026-07-25 — still live (~line 82); benign lint-level quirk; remains in this list, not ledger-tracked]**
10. **`any`-typed store fields** `emails`/`executives`/`moodEvents`/`weeklyOutcome`/`campaignResults` (`gameStore.ts:246-254`). **[TRIAGED 2026-07-25 — still live (now at `gameStore.ts:37-39` state + `:413-429` interface); remains in this list, not ledger-tracked]**
