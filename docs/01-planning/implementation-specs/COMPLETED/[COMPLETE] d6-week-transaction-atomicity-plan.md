...
> **Triaged 2026-07-25** (debt-cleanup session, C59): every item below carries a dated disposition. Three were promoted to real technical-debt-backlog entries (C108–C110); `[TRIAGED …]` items were re-verified live but deliberately stay in this list rather than the ledger.

1. `storage.updateProject` silently drops a tx argument two callers already pass — `server/storage.ts:371`, `shared/engine/processors/ProjectStageProcessor.ts:139`, `shared/engine/processors/TourProcessor.ts:212` (fixed incidentally by PR-2, but the underlying `any`-typed tx plumbing invites recurrence). **[RESOLVED — PR-2; `updateProject` carries the tx (`server/storage.ts:389`), re-verified 2026-07-25; the recurrence risk is now ledger-tracked as C110]**
2. `dbTransaction?: any` everywhere — no compile-time protection against exactly this class of bug (`server/storage.ts`, all processors). A typed `DbOrTx` union would prevent silent drops. **[PROMOTED C110 2026-07-25 — 113 occurrences verified across server/ + shared/]**
3. `processWeeklyCharts` swallows all errors → week can commit without chart rows — `shared/engine/game-engine.ts:1038–1041`. **[PROMOTED C108 2026-07-25 — swallow re-verified (catch now ~`game-engine.ts:1985`, also silently skips chart-milestone bonuses); same class as C55's email swallow]**
4. `applyArtistChangesToDatabase` runs twice per week — `shared/engine/game-engine.ts:191` and `:226`. **[BY-DESIGN — deliberate two-phase flush (meeting-phase flush so autonomous deltas land, then the post-processing flush), commented at the call sites (now `game-engine.ts:216-296`); verified 2026-07-25]**
5. `InvestmentTracker` idempotency flags read via non-tx `getRelease` — `shared/engine/FinancialSystem.ts:287,336` — check-then-set race (mitigated by PR-2 + PR-3 serialization). **[RESOLVED — the reads are now tx-threaded (`getRelease(releaseId, dbTransaction)`, `FinancialSystem.ts:350,399`) and the check-then-set is serialized under D6's `FOR UPDATE`; re-verified 2026-07-25]**
6. `ServerGameData` duplicates storage with inline SQL against hard-imported singletons — `server/data/gameData.ts:759–845, 1015–1036` — two parallel data layers to keep tx-aware. **[RESOLVED — `gameData.ts` no longer runs inline SQL; its data methods delegate to tx-aware `storage` calls; re-verified 2026-07-25]**
7. Extreme `console.log` volume in the hot path — `server/services/advanceWeekService.ts` (throughout), `server/storage.ts:384–425, 509–544`, `server/data/gameData.ts:760–818`. **[TRIAGED 2026-07-25 — still live (39/40/58 `console.log` in the three files); the ActionProcessor side (75 logs) was gated behind `DEBUG_ENGINE` by C66's safe slice (`6924597`); remains in this list, not ledger-tracked]**
8. `weekly_actions.results` splits week revenue/expenses evenly across actions — `server/services/advanceWeekService.ts:307` — meaningless per-action attribution. **[TRIAGED 2026-07-25 — still live (now `advanceWeekService.ts:453-455`); remains in this list, not ledger-tracked]**
9. `rngSeed` fallback `Math.random()` inside the service — `server/services/advanceWeekService.ts:87` — nondeterminism if a row lacks a seed. **[PROMOTED C109 2026-07-25 — verified live (now `advanceWeekService.ts:206`)]**
10. tx2's `gameStates` UPDATE has no optimistic week guard — `server/services/advanceWeekService.ts:270–295` (partially addressed by PR-3's `FOR UPDATE`; a `WHERE current_week = expected` rejection is the complete fix). **[RESOLVED — shipped as C58 (`3d8066a`, July 4, 2026): `expectedCurrentWeek` guard inside the `FOR UPDATE` lock, 409 `ADVANCE_WEEK_CONFLICT`; re-verified 2026-07-25]**
11. Hardcoded `week: gameState.currentWeek || 14` in the early-return summary — `server/services/advanceWeekService.ts:129`. **[TRIAGED 2026-07-25 — still live (now `advanceWeekService.ts:250`); remains in this list, not ledger-tracked]**
