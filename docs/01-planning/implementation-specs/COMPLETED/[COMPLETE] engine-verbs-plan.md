# [COMPLETE] Engine Verbs — Tier 1 + Tier 2 mechanism arc

> **Status:** COMPLETE — executed same-day 2026-07-12 on `content/meeting-content-session` (six parallel worktree branches merged in the planned order: slice-1 chained/scheduled events `38ba353`, `feat/ev-flags-keys`, `feat/ev-tangible-catalog`, `feat/ev-targeting`, `feat/ev-live-economy`, `feat/ev-requires-gates`). **Shipped:** 13 new effect keys (`schedule_event`, `story_flag`, `spawn_prospect`, `set_exec_absence`, `distribution_efficiency`, `press_scrutiny_flag`, `grant_song`, `spawn_release`, `promote_release`, `catalog_damage`, `cancel_project`, `grant_inventory`, `transfer_revenue_stream`) + 2 targeting directives (M13 `target_executive`, M14 per-artist event-targeting rider) + the M16 `requires` grammar (cash/week thresholds, story flags, artist-state tags — two-site lockstep helper, parity test `tests/engine/meeting-selection-two-site-parity.test.ts`) + the `flags.scheduled_events[]` queue with weekly promotion (workflow step 8b) + escalation last-seen (`flags.escalationHistory`, M12b) + exec absence (`flags.execAbsence`, M7) + the `STRUCTURED_EFFECT_KEYS` convention (sanctioned non-numeric effect values, ActionProcessor.ts:147). Suite 2,230 green ×2 (serial double-run); golden master re-blessed ONCE, audited — 5 lines, the escalation fixture gaining the `flags.escalationHistory` last-seen stamp (`8a34dc7`). **Deferred (per the list below, unchanged):** M8 multi-version releases; M6 restart/delay; full inventory tables; M17 new happening types; M18 mood prompt variants; M19 meeting cooldowns. **M14 partial:** the scheduled-events queue carries the scheduling choice's pinned `artistId` (game-engine.ts ~1128–1136), but threading the TRIGGERING meeting's resolved artist through `applyEscalation` for autonomous escalations remains a TODO at game-engine.ts ~1008 — predetermined directives on escalation events still resolve against the highest-popularity artist. All 13 keys are engine-live but content-unused today — authoring them into scenarios is the v3 content-integration wave (see C103/C97 in the debt ledger).
> **Original status:** READY — designer-authorized 2026-07-12 ("Execute ALL TIER 1 and TIER 2"), same session as the v3 content authoring. Root cause: 60% of the 98 authored v3 scenarios ship a compromised "honest version" because the 13-effect-key ceiling can't cash their fictions (Mac 87%, CEO 83%, escalations ~100% compromised; Pat 36%, side events 25%). Full analysis: three-agent Opus panel (demand census / stakes-impact ranking / feasibility+blast-radius), synthesized in this doc.
