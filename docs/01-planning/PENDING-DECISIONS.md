...
## 2. Hype & pre-marketing arc (Buzz v2) — schedule + forks
**Q:** When to run the "link the systems" hype redesign, and its six §5 forks (scoping model, label-pool consumption, expiry, pre-campaign returns curve, cancel semantics, lead-single role)? Full design: `implementation-specs/[DRAFT] hype-and-premarketing-plan.md`. **Already decided (July 6):** banked hype attaches at PLAN time. **Unblocks:** meeting-Buzz choices that mean what they say (today `ar_single_choice` picks an artist but banks to a label-global invisible pool); pre-release anticipation (makes early planning a strategy instead of a cash trap — interacts with C43); pilots effect-legibility directions B/C. **Defer cost:** the meeting Buzz channel stays untargeted/invisible/unattributed; plan→release stays dead air. **Recommendation:** next major arc after #149/#151 merge and playtest data settles; decide fork E together with C43 (same cancel-semantics territory).

## 3. C43 — cancel/reschedule planned releases
**Q:** Should planned releases be cancellable/reschedulable, and what's the refund rule? Server DELETE endpoint with safe refund already exists; only UI + refund-policy decision missing. **Unblocks:** Release-Experience Tier 3. **Defer cost:** money committed to a distant release week stays unrecoverable; inconsistent with tours (60% refund). *(July 6: consider deciding alongside the Buzz-v2 arc's fork E — cancelled releases' pre-built buzz semantics live in the same territory.)*
