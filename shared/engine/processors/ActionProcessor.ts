...
export class ActionProcessor {
  ...
  async processRoleMeeting(ctx: WeekContext, action: GameEngineAction, dbTransaction?: any): Promise<void> {
    ...
    if (choice.effects_immediate) {
      ...
      // Engine-verbs M1a/M1b: grant_song / spawn_release carry OBJECT values, which
      // the numeric filter above intentionally excludes (they are not badge
      // material and must never be mood-scaled). Thread them through to
      // applyEffects separately, preserving authored order (author grant_song
      // BEFORE spawn_release when pairing them — the spawn's 'granted' mode
      // consumes the song granted earlier in the same effects block). When no
      // tangible key is authored (all current content), effectsToApply IS
      // immediateToApply — byte-identical to the pre-slice call.
      const tangibleEffects: Record<string, any> = {};
      for (const [key, value] of Object.entries(choice.effects_immediate)) {
        if ((key === 'grant_song' || key === 'spawn_release') && value && typeof value === 'object') {
          tangibleEffects[key] = value;
        }
      }
      const effectsToApply = Object.keys(tangibleEffects).length > 0
        ? { ...immediateToApply, ...tangibleEffects }
        : immediateToApply;
      await this.applyEffects(ctx, effectsToApply, targetArtistId, targetScope, meetingName, choiceId); // Pass all context for logging
