...
**Open threads / next steps:**
- **(Resolved July 1, 2026) UI focus-slot cap** — `ActionSelectionPool.tsx` no longer hardcodes `>= 3`; it now uses a `focusSlots` prop with a base-3 fallback (backlog C29). Remaining follow-up: the component has **no render call site** in `client/src` (dead code) — wire it up or delete it.
