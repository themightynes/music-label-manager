...
- **Security wave, PR #62**: deleted the two zero-auth routes found by the reviews — `GET /api/debug/game/:gameId/revenue` and `POST /api/select-actions` (the latter already flagged as orphaned/dead in Comment from the July 1 follow-up) — and admin-gated `POST /api/dev/markets-config`.
