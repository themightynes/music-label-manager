...
- **TanStack Query owns EVERYTHING server-shaped**, including the `gameState` spine. Two kinds of cache-owned data:
  ...
  - **The `gameState` spine** (Phase 3.5): week, money, reputation, creativeCapital, focusSlots, access tiers, flags, A&R fields, embedded `musicLabel`. Lives at `['gameState:record', gameId]` (`gameStateQueryKey(gameId)` in `hooks/useGameState.ts`) as a **client-committed record**: `staleTime`/`gcTime: Infinity`, no focus/mount/reconnect refetch. The ONLY writer is `gameStore.ts`'s internal `commitGameState(set, next, previousGameId?)` funnel — every one of the store's 14 spine-writing paths (`loadGame`, `loadGameFromSave`, `createNewGame`, `selectAction`, `removeAction`, `clearActions`, `advanceWeek`, `cancelProject`, `consumeAROfficeSlot`, `releaseAROfficeSlot`, `adoptServerBalances`, `adoptServerSideEventResolution` (Tier 2 PR-4, backing the `resolveSideEvent` action), `adoptServerPlanReleaseResolution` (Buzz-v2 slice 2 — plan-release drains hype flags, so this sibling merges money+cc+`flags`), `cancelRelease` (Buzz-v2 slice 4 — adopts the server refund onto money)) goes through it. Do NOT add these arrays, or ad-hoc spine fields, back onto the Zustand store.
