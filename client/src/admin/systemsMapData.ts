...
export const EDGES: SystemEdge[] = [
  ...
  },
  {
    ...
    formula: `only attendance >${tourRevenue.popularity_reactions.attendance_threshold}%: gain scaled by crowd size (${tourRevenue.popularity_reactions.tiers.map((t: any) => t.gain).join('..+')})`,
    values: [
    ...
    note: 'Knob liberation (slice 1): attendance→popularity reaction table moved to markets.json tour_revenue.popularity_reactions.',
  },
  {
    id: 'e-streams-chart',
    from: 'streams',
