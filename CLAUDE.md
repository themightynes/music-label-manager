...
- **Doc placement — the "does `main` lie?" test** (2026-07-27 ruling; supersedes the prior two-tier `docs/session-<date>` rule). For any cross-session doc, ask: *if this lands on `main` while its related code sits in an unmerged PR, does `main` now assert something false?*
  - **No → commit it straight to `main`** (no branch, no PR): `DEVELOPMENT_STATUS.md` (a journal — it *narrates* state and may reference open PRs), `ai_instructions.md`, and `CLAUDE.md`/`docs/CLAUDE.md` convention changes (evergreen). **Never put `DEVELOPMENT_STATUS.md` on a feature branch** — that's what made `main`'s status go stale and forced the old "check for an active session branch" dance. Keeping it main-only also means feature branches never conflict on it.
  - **Yes → the doc rides its code PR** (so `main` never claims work is done before it merges): `docs/09-troubleshooting/technical-debt-backlog.md` (a ledger — "resolved/pending" must match merged code), *implemented* `PENDING-DECISIONS.md` rulings, descriptive docs (`docs/02-architecture/`, `03-workflows/`, `04-frontend/`, `05-backend/`, reference docs), and `tasks/*`. **Exception:** a *pure no-code decision* (e.g. "defer C32 indefinitely") is true the moment it's made, so it may go straight to `main`.
  - **Net effect:** `main`'s `DEVELOPMENT_STATUS.md` is always current, so `/onboard` just reads it — no stray-docs-PR check needed — while `main` never lies about merged state. (Context: this project is run by a solo game designer through the LLM, who makes the design rulings; the engineering/git/verification ceremony is the assistant's to own and keep low.)
